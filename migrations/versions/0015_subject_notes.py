...
def upgrade():
    ...
    op.create_index("ix_notes_location_subject", "notes", ["location_id", "subject_type", "subject_id", "created_at"])
    bind = op.get_bind()
    if bind.dialect.name == "postgresql":
        op.execute('ALTER TABLE "notes" ENABLE ROW LEVEL SECURITY')
        op.execute('ALTER TABLE "notes" FORCE ROW LEVEL SECURITY')
        op.execute("""CREATE POLICY "notes_location_isolation" ON "notes" USING (location_id = NULLIF(current_setting('app.location_id', true), '')::integer) WITH CHECK (location_id = NULLIF(current_setting('app.location_id', true), '')::integer)""")
...
def downgrade():
    ...
    if bind.dialect.name == "postgresql":
        ...
        op.execute('DROP POLICY IF EXISTS "notes_location_isolation" ON "notes"')
    op.drop_index("ix_notes_location_subject", table_name="notes")
    op.drop_table("notes")
