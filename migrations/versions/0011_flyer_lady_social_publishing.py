...
def upgrade():
    ...
    if "meta_social_oauth_sessions" not in existing:
        op.create_table("meta_social_oauth_sessions",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("location_id", sa.Integer(), sa.ForeignKey("locations.id", ondelete="CASCADE"), nullable=False),
    ...
    if "meta_social_connections" not in existing:
        op.create_table("meta_social_connections",
            ...
            sa.UniqueConstraint("location_id", name="uq_meta_social_connection_location"))
    if not any(i["name"] == "ix_meta_social_oauth_location_status" for i in inspector.get_indexes("meta_social_oauth_sessions")):
        op.create_index("ix_meta_social_oauth_location_status", "meta_social_oauth_sessions", ["location_id", "status"])

    if "flyer_lady_specials" not in existing:
        op.create_table("flyer_lady_specials",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("location_id", sa.Integer(), sa.ForeignKey("locations.id", ondelete="CASCADE"), nullable=False), sa.Column("created_by", sa.String(100), nullable=False), sa.Column("text", sa.Text(), nullable=False),
        ...
        op.create_index("ix_flyer_specials_location_status", "flyer_lady_specials", ["location_id", "status"])
        op.create_index("ix_flyer_specials_location_created", "flyer_lady_specials", ["location_id", "created_at"])
    if "flyer_lady_special_approvals" not in existing:
        op.create_table("flyer_lady_special_approvals",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("location_id", sa.Integer(), sa.ForeignKey("locations.id", ondelete="CASCADE"), nullable=False), sa.Column("special_id", sa.Integer(), sa.ForeignKey("flyer_lady_specials.id", ondelete="CASCADE"), nullable=False),
        ...
        op.create_index("ix_flyer_approvals_location_special", "flyer_lady_special_approvals", ["location_id", "special_id", "decided_at"])
    if "flyer_lady_special_posts" not in existing:
        op.create_table("flyer_lady_special_posts",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("special_id", sa.Integer(), sa.ForeignKey("flyer_lady_specials.id", ondelete="CASCADE"), nullable=False), sa.Column("location_id", sa.Integer(), sa.ForeignKey("locations.id", ondelete="CASCADE"), nullable=False),
        ...
        op.create_index("ix_flyer_posts_location_status", "flyer_lady_special_posts", ["location_id", "status"])
    ...
    if "flyer_lady_public_links" not in existing:
        op.create_table("flyer_lady_public_links",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("special_id", sa.Integer(), sa.ForeignKey("flyer_lady_specials.id", ondelete="CASCADE"), nullable=False, unique=True), sa.Column("location_id", sa.Integer(), sa.ForeignKey("locations.id", ondelete="CASCADE"), nullable=False), sa.Column("target_url", sa.String(2000), nullable=False), sa.Column("active", sa.Boolean(), nullable=False, server_default=sa.true()), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False))
    if "flyer_lady_link_clicks" not in existing:
        op.create_table("flyer_lady_link_clicks",
            sa.Column("id", sa.Integer(), primary_key=True), sa.Column("special_id", sa.Integer(), sa.ForeignKey("flyer_lady_specials.id", ondelete="CASCADE"), nullable=False), sa.Column("location_id", sa.Integer(), sa.ForeignKey("locations.id", ondelete="CASCADE"), nullable=False), sa.Column("user_agent", sa.String(1000)), sa.Column("referrer", sa.String(2000)), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False))
        op.create_index("ix_flyer_clicks_special_created", "flyer_lady_link_clicks", ["special_id", "created_at"])
        op.create_index("ix_flyer_clicks_location_created", "flyer_lady_link_clicks", ["location_id", "created_at"])
...
def downgrade():
    ...
    for index, table in [("ix_flyer_clicks_location_created","flyer_lady_link_clicks"),("ix_flyer_clicks_special_created","flyer_lady_link_clicks"),("ix_flyer_posts_special","flyer_lady_special_posts"),("ix_flyer_posts_location_status","flyer_lady_special_posts"),("ix_flyer_approvals_location_special","flyer_lady_special_approvals"),("ix_flyer_specials_location_created","flyer_lady_specials"),("ix_flyer_specials_location_status","flyer_lady_specials")]:
