...
def upgrade():
    ...
    if "review_platform" not in columns:
        op.add_column("tenants", sa.Column("review_platform", sa.String(30), nullable=True))
    if "review_url" not in columns:
        op.add_column("tenants", sa.Column("review_url", sa.String(1000), nullable=True))
    ...
    if "review_message_template" not in columns:
        op.add_column("tenants", sa.Column("review_message_template", sa.Text(), nullable=True))

    if bind.dialect.name == "postgresql":
        op.execute("ALTER TABLE tenants DROP CONSTRAINT IF EXISTS ck_tenant_review_platform")
        op.execute("""
            ALTER TABLE tenants
