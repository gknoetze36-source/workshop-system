...
def upgrade():
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("location_id", "booking_id", name="uq_booking_confirmation_location_booking"),
        sa.CheckConstraint("decision IN ('confirmed','declined')", name="ck_booking_confirmation_decision"),
    )
    op.create_index(
        "ix_booking_confirmations_location_customer_decided",
        "booking_confirmations", ["location_id", "customer_id", "decided_at"]
    )
    bind = op.get_bind()
    if bind.dialect.name == "postgresql":
        ...
        op.execute("""
            CREATE POLICY booking_confirmations_location_isolation
...
def downgrade():
    ...
    if bind.dialect.name == "postgresql":
        ...
        op.execute("DROP POLICY IF EXISTS booking_confirmations_location_isolation ON booking_confirmations")
    op.drop_index("ix_booking_confirmations_location_customer_decided", table_name="booking_confirmations")
