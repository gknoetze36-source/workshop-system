...
- **Client/vehicle-owner data (Customer's end customers):** name, phone number, vehicle details, service and booking history, WhatsApp message content and metadata (timestamps, delivery status), and marketing consent status and its evidence (when, by what means, and from what source consent or an opt-out was recorded).
- **Payment data:** transaction references, payment status, amounts, and Plan details. Card and bank details are collected and stored directly by our payment processor, Paystack — we do not store full card numbers or card security codes.
- **Usage and technical data:** log data, device/browser information, IP address, and diagnostic data needed to operate and secure the Service.
- **Security data:** records of authentication events (sign-in, sign-out, failed sign-in attempts, password changes) held for the security of the Service. Where a submitted sign-in identifier does not match a known account, we store only a one-way keyed hash of it, never the value itself.
- **Communications:** support requests and correspondence with us.

## 3. Why we process this information (lawful basis)
We rely on one or more of the following, per POPIA's processing conditions:
- **Performance of a contract** — to provide the Account, process bookings, and bill for the Service.
- **Consent** — for marketing WhatsApp messaging to end customers (obtained by the Customer, see clause 8) and for optional marketing communications.
- **Legitimate interests** — to secure, maintain, and improve the Service, and to prevent fraud, balanced against data subjects' rights.
- **Legal obligation** — e.g. financial record-keeping under FICA/tax law.
...
You may opt out of marketing messages at any time by replying **STOP** to a WhatsApp message, by using the unsubscribe option on a marketing message, or by contacting the workshop directly. Opting out of marketing **does not** stop the service messages about your booking or vehicle described above; you will receive a confirmation explaining this. You may resume marketing messages by replying START.

## 8. Operator relationship — data belonging to our Customers' clients
Where a workshop's own customer's personal information flows through PHANTA (e.g. a vehicle owner's phone number and service history), the **workshop is the Responsible Party** and **PHANTA acts as its Operator**, processing that information only on the workshop's documented instructions, under the terms of our **Data Processing Agreement**. Vehicle owners with questions about how their data is used should contact the workshop directly in the first instance.

## 9. Who we share information with
We share personal information with the following categories of recipients, each bound by contract to protect it:
- **Meta / WhatsApp Business Platform** — to deliver messages you or the workshop send/receive.
- **Paystack** — to process payments (South African payment service provider).
- **Google Business Profile** — where enabled, to manage the workshop's business listing and review requests. PHANTA does not integrate with Google Calendar.
- **AI model providers (currently OpenAI)** — to generate automated message drafts and content. Only the information needed for the specific conversation is transmitted: the relevant customer's name, their vehicles, their open bookings, and the recent conversation. Whole customer databases, other workshops' data, credentials, and payment information are not transmitted.
- **Hosting provider (Railway)** — to host the Service infrastructure and database.
- **Sentry** — to receive application error and diagnostic reports so we can detect and fix faults. Personal information is suppressed before transmission, and request bodies, cookies, and authentication headers are stripped.
...
## 12. Data breaches
If we become aware of a reasonable belief that personal information has been accessed or acquired by an unauthorised person, we will notify the Information Regulator and affected data subjects (or the affected Customer, where PHANTA acts as Operator) as soon as reasonably possible, as required by POPIA section 22. We maintain an internal incident register to record what occurred, which systems and Customers were affected, and which records may have been involved, so that notifications can be made accurately.
